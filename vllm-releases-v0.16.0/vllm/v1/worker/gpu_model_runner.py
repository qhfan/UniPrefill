...
from vllm.forward_context import (
    BatchDescriptor,
    set_forward_context,
    is_forward_context_available,  # 新增
    get_forward_context
)
...
def _get_inner_model(model):
    # Gemma3 multimodal: model.language_model.model
    # Gemma3 text-only:  model.model
    # LLaMA:             model.model
    for path in [
        lambda m: m.language_model.model,  # Gemma3 multimodal
        lambda m: m.model,                  # LLaMA / Gemma3 text-only
    ]:
        try:
            candidate = path(model)
            if hasattr(candidate, 'drop_layer_indices'):
                return candidate
...
class GPUModelRunner(
    ...
    def __init__(
        ...
        self._token_drop_per_req: dict[str, list[tuple[int, int]]] = {}
    ...
    def _update_states(self, scheduler_output: "SchedulerOutput") -> None:
        ...
        for req_id in scheduler_output.finished_req_ids:
            ...
            self._token_drop_per_req.pop(req_id, None)
    ...
    def execute_model(
        ...
        # logger.info(
        #     f"[DEBUG-MODEL-STRUCTURE] "
        #     f"self.model={type(self.model)} "
        #     f"self.model.model={type(self.model.model) if hasattr(self.model, 'model') else 'NO'} "
        #     f"self.model.model.model={type(self.model.model.model) if hasattr(self.model, 'model') and hasattr(self.model.model, 'model') else 'NO'} "
        #     f"self.model.language_model={type(self.model.language_model) if hasattr(self.model, 'language_model') else 'NO'} "
        # )
        ...
        with (
            ...
            # 当前 step 的 req_ids，顺序与 seq_lens / query_start_loc 完全一致
            _fwd_ctx._req_ids_snapshot: list[str] = [
                self.input_batch.req_ids[i] for i in range(_num_reqs)
            ]
            _llama_model = self.model
            if hasattr(_llama_model, 'language_model'):
                _llama_model = _llama_model.language_model
            if hasattr(_llama_model, 'model'):
                _llama_model = _llama_model.model

            _has_drop_config = (
                hasattr(_llama_model, 'drop_layer_indices')
                and len(_llama_model.drop_layer_indices) > 0
            )

            # if _has_drop_config:
            #     logger.info(
            #         f"[DEBUG-RUNNER] model_type={type(self.model)} "
            #         f"has_model={hasattr(self.model, 'model')} "
            #         f"_llama_model_type={type(_llama_model)} "
            #         f"has_drop_layer_indices={hasattr(_llama_model, 'drop_layer_indices')} "
            #         f"_has_drop_config={_has_drop_config} "
            #         f"_token_drop_per_req_len={len(self._token_drop_per_req)}"
            #     )


            if _has_drop_config:
                _fwd_ctx._is_mixed_or_prefill_step = False
                if isinstance(_fwd_ctx.attn_metadata, dict):

                    _first_flash_meta = None
                    for _m in _fwd_ctx.attn_metadata.values():
                        if hasattr(_m, 'max_query_len') and hasattr(_m, 'block_table'):
                            _first_flash_meta = _m
                            break
                    if _first_flash_meta is not None \
                            and _first_flash_meta.max_query_len > 1:
                        _fwd_ctx.skip_compiled = True
                        _fwd_ctx._is_mixed_or_prefill_step = True

            

            # if len(self._token_drop_per_req) > 0:
            #     logger.info(
            #         f"[DEBUG-RUNNER] _token_drop_per_req keys={list(self._token_drop_per_req.keys())[:3]} "
            #         f"_req_ids_cur={_fwd_ctx._req_ids_snapshot[:3]} "
            #         f"_any_in={any(rid in self._token_drop_per_req for rid in _fwd_ctx._req_ids_snapshot)}"
            #     )
            ...
            if len(self._token_drop_per_req) > 0:
                if isinstance(_fwd_ctx.attn_metadata, dict):
                    ...
                    if _meta0 is None:
                        ...
                        _global_kv_seq_lens = _meta0.seq_lens
                        _block_table        = _meta0.block_table
                        _device             = _global_kv_seq_lens.device
                        _bs                 = self.cache_config.block_size

                    _req_ids_cur = _fwd_ctx._req_ids_snapshot    


                    _any_drop = any(rid in self._token_drop_per_req for rid in _req_ids_cur)
                    
                    if _any_drop:
                        _llama_model_inner = self.model
                        if hasattr(_llama_model_inner, 'language_model'):
                            _llama_model_inner = _llama_model_inner.language_model
                        if hasattr(_llama_model_inner, 'model'):
                            _llama_model_inner = _llama_model_inner.model
                        _drop_indices = set(getattr(_llama_model_inner, 'drop_layer_indices', []))
                        _first_drop = min(_drop_indices) if _drop_indices else None

                        if _first_drop is not None:
                            _config = _llama_model_inner.config
                            _total_layers = len(getattr(_config, 'layer_types', [])) \
                                or _config.num_hidden_layers

                            _affected_layers = sorted([
                                _li for _li in range(_total_layers)
                                if _li > _first_drop
                            ])

                            # _layer_types = getattr(_config, 'layer_types', None)
                            # _affected_layers = sorted([
                            #     _li for _li in range(_total_layers)
                            #     if _li > _first_drop
                            #     and (_layer_types is None 
                            #         or _li >= len(_layer_types) 
                            #         or _layer_types[_li] != 'sliding_attention')
                            # ])


                            _layer_block_table: dict[int, torch.Tensor] = {}
                            _layer_sw: dict[int, int] = {}

                            for _lname, _meta in _fwd_ctx.attn_metadata.items():
                                _li = None
                                _parts = _lname.split('.')
                                for _pi, _pp in enumerate(_parts):
                                    if _pp == 'layers' and _pi + 1 < len(_parts):
                                        try:
                                            _li = int(_parts[_pi + 1])
                                        except ValueError:
                                            pass
                                        break
                                if _li is not None and _li not in _layer_block_table:
                                    if hasattr(_meta, 'block_table') and _meta.block_table is not None:
                                        _layer_block_table[_li] = _meta.block_table
                                    _lt = getattr(_config, 'layer_types', None)
                                    if _lt is not None and _li < len(_lt) \
                                            and _lt[_li] == 'sliding_attention':
                                        _layer_sw[_li] = getattr(_config, 'sliding_window', 0)
                                    else:
                                        _layer_sw[_li] = 0

                            if _affected_layers:
                                _per_layer_seqused: list[tuple[int, torch.Tensor]] = []
                                _per_layer_sm: dict[int, torch.Tensor] = {}

                                for _fl in _affected_layers:
                                    _seqused = torch.empty(
                                        _num_reqs, dtype=torch.int32, device=_device
                                    )

                                    for _i, _rid in enumerate(_req_ids_cur):
                                        _cur_kv = int(_global_kv_seq_lens[_i].item())

                                        if _rid in self._token_drop_per_req:
                                            _entries = self._token_drop_per_req[_rid]
                                            _orig_len = _entries[0][1]
                                            _sl_at_fl = _orig_len
                                            for _el, _ev in _entries[1:]:
                                                if _el < _fl:
                                                    _sl_at_fl = _ev
                                                else:
                                                    break
                                            _decode_steps = max(0, _cur_kv - _orig_len)
                                            _seqused[_i] = _sl_at_fl + _decode_steps
                                        else:
                                            _seqused[_i] = _cur_kv

                                    _per_layer_seqused.append((_fl, _seqused))

                                    _bt = _layer_block_table.get(_fl, _block_table)
                                    _sw = _layer_sw.get(_fl, 0)

                                    _lpos = (_seqused - 1).long()
                                    # if _sw > 0:
                                    #     _lpos = _lpos % _sw
                                    _blks = _lpos // _bs
                                    _offs = _lpos % _bs
                                    _preqs = torch.arange(_num_reqs, device=_device)
                                    _pblks = _bt[_preqs, _blks]
                                    _per_layer_sm[_fl] = (
                                        _pblks * _bs + _offs
                                    ).to(torch.int64)

                                _fwd_ctx._decode_per_layer_seq_lens = _per_layer_seqused
                                _fwd_ctx._decode_per_layer_slot_mapping = _per_layer_sm
            ...
            _fwd_ctx = get_forward_context()
            if getattr(_fwd_ctx, 'token_drop_applied', False) \
                    and _fwd_ctx.new_cu_seqlens is not None:

                logits_indices = (_fwd_ctx.new_cu_seqlens[1:] - 1).to(torch.int64)
                ...
                _drop_dict: dict[str, list[tuple[int, int]]] | None = getattr(
                    _llama_model, 'per_layer_seq_lens', None
                )
                if isinstance(_drop_dict, dict):

                    for _rid, _entries in _drop_dict.items():
                        self._token_drop_per_req[_rid] = _entries
